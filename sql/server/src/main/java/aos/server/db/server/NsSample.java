...
/**
 * The Network Server sample demo program is a simple JDBC application that
 * interacts with the Derby Network Server. The program:
 * 
 * 1. starts the Derby Network Server 2. loads the IBM DB2 JDBC Universal driver
 * 3. creates the database if not already created 4. checks to see if the schema
 * is already created, and if not, 5. creates the schema which includes the
 * table SAMPLETBL and corresponding indexes. 6. connects to the database 7.
 * loads the schema by inserting data 8. starts client threads to perform
 * database related operations 9. has each of the clients perform DML operations
 * (select, insert, delete, update) using JDBC calls, i) one client opens an
 * embedded connection to perform database operations You can open an embedded
 * connection in the same JVM that starts the Derby Network Server. ii) one
 * client opens a client connection to the Derby Network Server to perform
 * database operations. 10.waits for the client threads to finish the tasks
 * 11.shuts down the Derby Network Server at the end of the demo
 * 
 * <P>
 * Usage: java nserverdemo.NsSample
 * <P>
 * Please note, a file derby.log is created in the directory you run this
 * program. This file contains the logging of connections made with the derby
 * network server
 */
...
public class NsSample {
    ...
    // String("org.apache.derby.jdbc.EmbeddedDriver");
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            /*
             * Start - In order to start the network server do the following In
             * case you want to start the server as a script or another program
             * comment out the next block of code (i.e. until the comment line
             * 'End - network server started') Also, comment out the 'Shutdown
             * Derby Network Server' line of code at the bottom In case you
             * decide to comment out the starting of the network server, make
             * sure that the client thread is not making an embedded connection
             * but instead making only a client connection. Also note, the
             * server logs messages to the file derby.log in the directory you
             * run this program
             */

            {
                ...
                boolean knowIfServerUp = false; // do we know if server is ready
                                                // to accept connections
                int numTimes = 5;

                // Test to see if server is ready for connections, for 15
                // seconds.
                while (!knowIfServerUp && (numTimes > 0)) {
                    try {
                        // testing for connection to see if the network server
                        // is up and running
                        // if server is not ready yet, this method will throw an
                        // exception
                        numTimes--;
                        ...
                        System.out
                                .println("[NsSample] Unable to obtain a connection to network server, trying again after 3000 ms.");
            ...
            // See Derby documentation for description of properties that may be
            // set
            ...
            // Get database connection using the JCC client via DriverManager
            // api
            try {
            ...
            NsSampleWork.checkAndCreateSchema(conn, pw); // Check and create the
                                                         // necessary schema if
                                                         // not already created
            NsSampleWork.loadSchema(conn, NUM_ROWS, pw); // Insert rows into the
                                                         // table
            conn.close();
            ...
            /*
             * Only the JVM that starts the Derby Network Server can obtain an
             * embedded connection Please pay attention to the database URL
             * Also, you need not load the org.apache.derby.jdbc.EmbeddedDriver
             * since it is already loaded when the network server starts up. 1.
             * Derby embedded database url - jdbc:derby:databasename
             */
            clientThreads[0] = new NsSampleClientThread(1, CS_EMBED_DBURL, properties, pw);
            clientThreads[0].start();

            /*
             * 2. The below client threads obtain a client connection to Derby
             * Network Server One can also get a client connection from another
             * JVM Please be aware of the database URL for obtaining a client
             * connection
             */
