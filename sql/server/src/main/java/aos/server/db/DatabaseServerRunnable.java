...
public class DatabaseServerRunnable implements Runnable {
	 ...
	 */
	public DatabaseServerRunnable(InetAddress address, int portNumber,
	 ...
	 * @throws			   Exception on error
	 */
	public DatabaseServerRunnable(String userName, String password)
			throws Exception
	{
	 ...
	 * @throws             Exception on error
	 */
	public DatabaseServerRunnable(InetAddress address,int portNumber) throws Exception
	{
		serverImpl = new NetworkServerControlImpl(address, portNumber);
	}
	 ...
	 * @throws             Exception on error
	 */
	public DatabaseServerRunnable() throws Exception
	{
	...
	public void run() {
        ...
        try
        {
            server = new NetworkServerControlImpl(InetAddress.getByName("0.0.0.0"), 1527, "aos", "aos");
...
//        while (true) Thread.sleep(1000);
    ...
    private static  String  getCodeSourcePrefix( NetworkServerControlImpl server )
        throws Exception
    {
        String  derbyNetURL = DatabaseServerRunnable.class.getProtectionDomain().getCodeSource().getLocation().toExternalForm();
    ...
    {
        String      resourceName =
            DatabaseServerRunnable.class.getPackage().getName().replace( '.', '/' ) +
            '/' +
            POLICY_FILENAME;
        URL         resourceURL = DatabaseServerRunnable.class.getClassLoader().getResource( resourceName );
