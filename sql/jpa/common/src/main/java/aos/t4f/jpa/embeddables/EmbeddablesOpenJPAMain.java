...
public class EmbeddablesOpenJPAMain {
    ...
    public static void runQueries(EntityManager em) {
        ...
        q = em.createQuery("SELECT DISTINCT u FROM User u  WHERE u.contactInfo.homePhone.number='507-555-5555'"
        ...
        // Find users who's conatctInfo cell phone is 507-555-5555
        // Entity -> embedded -> embedded
        q = em.createQuery("SELECT u FROM User u " + "WHERE u.contactInfo.homePhone.number='507-555-5555'"
                + "AND u.contactInfo.homePhone.type='cell'");
        res = q.getResultList();
        for (User b : res) {
            System.out.println(b);
