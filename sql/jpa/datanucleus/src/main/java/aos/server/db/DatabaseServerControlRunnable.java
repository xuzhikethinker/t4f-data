...
public class DatabaseServerControlRunnable implements Runnable {
	 ...
	 */
	public DatabaseServerControlRunnable(InetAddress address, int portNumber,
	 ...
	 * @throws			   Exception on error
	 */
	public DatabaseServerControlRunnable(String userName, String password)
			throws Exception
	{
	 ...
	 * @throws             Exception on error
	 */
	public DatabaseServerControlRunnable(InetAddress address,int portNumber) throws Exception
	{
		serverImpl = new NetworkServerControlImpl(address, portNumber);
	}
	 ...
	 * @throws             Exception on error
	 */
	public DatabaseServerControlRunnable() throws Exception
	{
...
//        while (true) Thread.sleep(1000);
    ...
    private static  String  getCodeSourcePrefix( NetworkServerControlImpl server )
        throws Exception
    {
        String  derbyNetURL = DatabaseServerControlRunnable.class.getProtectionDomain().getCodeSource().getLocation().toExternalForm();
    ...
    {
        String      resourceName =
            DatabaseServerControlRunnable.class.getPackage().getName().replace( '.', '/' ) +
            '/' +
            POLICY_FILENAME;
        URL         resourceURL = DatabaseServerControlRunnable.class.getClassLoader().getResource( resourceName );
