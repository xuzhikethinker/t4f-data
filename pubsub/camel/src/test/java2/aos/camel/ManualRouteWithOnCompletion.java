...
import aos.camel.StopRouteProcessor;

import camelinaction.inventory.UpdateInventoryInput;

/**
 * A maintenance route which must be started manually to force updating the
 * inventory when a file is dropped into a special folder.
 * <p/>
...
public class ManualRouteWithOnCompletion extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("file://target/inventory/manual?maxMessagesPerPoll=1")
		        ...
		        .routeId("manual").noAutoStartup().log("Doing manual update with file ${file:name}")
		        .split(body().tokenize("\n")).convertBodyTo(UpdateInventoryInput.class).to("direct:update").end();
